...
import { checkResults } from "./services/result-checker";
import { saveMatchResults, saveWeeklyAccuracy, markPredictionsResolved, getWeekResults, getAllWeeklyAccuracy, getAccuracyOverview } from "./services/result-store";
...
export function buildRoutes(userRepository: UserRepository): Router {
  ...
  const processResultsBody = z.object({
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const parsed = processResultsBody.safeParse(req.body);
      if (!parsed.success) {
        return res.status(400).json({ error: "invalid_body", detail: parsed.error.flatten() });
      }
      const { bulletinId, gameRoundId } = parsed.data;

      if (!supabaseAdminClient) {
        return res.status(500).json({ error: "server_misconfigured", detail: "supabase_env_missing" });
      }

      // 1. Fetch predictions for this bulletin
      const { data: predRows, error: predError } = await supabaseAdminClient
        .from("predictions")
      ...
      if (predError) {
        return res.status(500).json({ error: "database_error", detail: predError.message });
      }
      if (!predRows || predRows.length === 0) {
        return res.status(404).json({ error: "not_found", detail: `No predictions for bulletin ${bulletinId}` });
      }

      // 2. Check results against Spor Toto API
      const output = await checkResults(bulletinId, gameRoundId, predRows as { match_id: number; prediction: string }[]);

      // 3. Save to Supabase
      await saveMatchResults(bulletinId, gameRoundId, output.results);
      await saveWeeklyAccuracy(output);
      await markPredictionsResolved(supabaseAdminClient, bulletinId);

      return res.json({
        ok: true,
        bulletinId,
        totalMatches: output.totalMatches,
        correctPredictions: output.correctPredictions,
        accuracyPct: output.accuracyPct,
  ...
  router.get(
    "/results/weekly",
    asyncHandler(async (_req, res) => {
      const data = await getAllWeeklyAccuracy();
  ...
  router.get(
    "/results/overview",
    asyncHandler(async (_req, res) => {
      const data = await getAccuracyOverview();
  ...
  router.get(
    "/results/:bulletinId",
    asyncHandler(async (req, res) => {
      const bulletinId = Array.isArray(req.params.bulletinId) ? req.params.bulletinId[0] : req.params.bulletinId;
      const data = await getWeekResults(bulletinId);
      if (!data.length) {
        return res.status(404).json({ error: "not_found", detail: `No results for ${bulletinId}` });
      }
      return res.json({ bulletinId, results: data });
    })
  );
